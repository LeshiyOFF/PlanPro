...
Все файлы для установки приложения находятся в папке **`release/`**:
- **Windows** — установщики и портативная сборка в `release/` (например, `.exe`, `win-unpacked/`)
- **Linux** — пакеты и образы в `release/` (например, `.deb`, `.rpm`, `.AppImage`)

Папка создаётся после выполнения команды `npm run dist` или после успешной сборки через CI/CD.
