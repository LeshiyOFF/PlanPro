 ...
 */
export const useActionExecution = (actionManager: IActionManager) => {
  ...
  const executeAction = useCallback(async (actionId: string): Promise<void> => {
    ...
    try {
      ...
      logger.error(`Failed to execute action ${actionId}:`, error instanceof Error ? { message: error.message } : { message: String(error) });
      throw error;
    } finally {
