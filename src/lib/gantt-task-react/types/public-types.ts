...
/**
 * Пропсы для кастомного хедера диаграммы Ганта
 * Передаются синхронно из библиотеки для точной синхронизации
 */
export interface CustomHeaderRenderProps {
  /** Начальная дата временной шкалы */
  startDate: Date;
  /** Количество интервалов (колонок) */
  datesLength: number;
  /** Ширина одной колонки в пикселях */
  columnWidth: number;
  /** Текущая позиция горизонтального скролла */
  scrollLeft: number;
  /** Текущий режим отображения (День/Неделя/Месяц и т.д.) */
  viewMode: ViewMode;
  /** Высота хедера */
  headerHeight: number;
}
...
/**
 * Тип для render prop кастомного хедера
 */
export type RenderCustomHeader = (props: CustomHeaderRenderProps) => ReactNode;
...
export interface GanttProps extends EventOption, DisplayOption, StylingOption {
  ...
  /**
   * Width of a single column/time unit in pixels (overrides distances.columnWidth)
   */
  columnWidth?: number;
  ...
  /**
   * Height of the header in pixels (overrides distances.headerHeight)
   */
  headerHeight?: number;
  /**
  ...
  tasks: readonly TaskOrEmpty[];
  /**
   * Render prop для кастомного хедера диаграммы
   * При наличии отключает встроенный Calendar компонент
   * Хедер рендерится вне scroll-контейнера для sticky поведения
   */
  renderCustomHeader?: RenderCustomHeader;
  /**
   * Height of a single row in pixels (overrides distances.rowHeight)
   */
  rowHeight?: number;
  /**
   * GANTT-NAV-V3: Минимальная дата для отображения (расширяет диапазон влево)
   * Позволяет навигировать до этой даты даже если нет задач
   */
  minDate?: Date;
  /**
   * GANTT-NAV-V3: Максимальная дата для отображения (расширяет диапазон вправо)
   * Позволяет навигировать до этой даты даже если нет задач
   */
  maxDate?: Date;
}
