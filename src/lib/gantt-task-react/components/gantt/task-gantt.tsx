...
export type TaskGanttProps = {
  ...
  /** Флаг наличия кастомного хедера - отключает встроенный Calendar */
...
const TaskGanttInner: React.FC<TaskGanttProps> = (props) => {
  const {
    ...
    ganttSVGRef,
    gridProps,
    distances: { columnWidth, rowHeight, minimumRowDisplayed, headerHeight },
    ganttTaskContentRef,
  ...
  // minWidth гарантирует что flex-контейнер не сожмёт дочерний элемент
  // Это позволяет горизонтальному скроллу работать корректно в flex-direction: column
  ...
  return (
    <div
      ...
      dir="ltr"
    >
      {/* Calendar скрывается при наличии кастомного хедера */}
      {!hasCustomHeader && <Calendar {...calendarProps} colors={colors} />}
