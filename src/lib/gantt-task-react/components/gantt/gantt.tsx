...
import { getMapTaskToGlobalIndex } from "../../helpers/get-map-task-to-overall-index";
...
export const Gantt: React.FC<GanttProps> = ({
                                              ...
                                              // GANTT-NAV-V3: Props для расширения диапазона навигации
                                              minDate,
                                              maxDate
                                            }) => {
  ...
  // GANTT-NAV-V2: Ref для отслеживания последнего обработанного viewDate
  // Используем ref вместо state чтобы избежать блокировки повторной навигации
  const lastProcessedViewDateRef = useRef<number | null>(null);
  ...
  const distances = useMemo<Distances>(
    () => ({
      ...defaultDistances,
      ...distancesProp,
      // Пропсы верхнего уровня переопределяют distances
      ...(columnWidthProp !== undefined && { columnWidth: columnWidthProp }),
      ...(headerHeightProp !== undefined && { headerHeight: headerHeightProp }),
      ...(rowHeightProp !== undefined && { rowHeight: rowHeightProp })
    }),
    [distancesProp, columnWidthProp, headerHeightProp, rowHeightProp]
  );
  ...
  // GANTT-NAV-V3: Передаём minDate/maxDate для расширения диапазона навигации
  const [startDate, minTaskDate, datesLength] = useMemo(
    () => ganttDateRange(visibleTasks, viewMode, preStepsCount, minDate, maxDate),
    [visibleTasks, viewMode, preStepsCount, minDate, maxDate]
  );
  ...
  // Пропсы для кастомного хедера (синхронные данные из библиотеки)
  const customHeaderProps = useMemo<CustomHeaderRenderProps>(
    () => ({
      startDate,
      datesLength,
    ...
    [startDate, datesLength, distances.columnWidth, scrollX, viewMode, distances.headerHeight]
  );

  // Создание элемента кастомного хедера через render prop
  const customHeaderElement = useMemo(
    () => renderCustomHeader?.(customHeaderProps) ?? null,
    [renderCustomHeader, customHeaderProps]
  );

  // Флаг наличия кастомного хедера для отключения встроенного Calendar
  const hasCustomHeader = renderCustomHeader !== undefined;
  ...
  // GANTT-NAV-V4: Обработка viewDate для навигации
  // Синхронный скролл как в оригинальной библиотеке wamra/gantt-task-react
  useEffect(() => {
    if (!viewDate) return;

    const viewDateValue = viewDate.valueOf();

    // Пропускаем если дата не изменилась (предотвращает лишние скроллы)
    if (lastProcessedViewDateRef.current === viewDateValue) return;

    // Вычисляем индекс колонки для скролла
    const index = getDatesDiff(viewDate, startDate, viewMode);

    // Критическая проверка из оригинала: если индекс отрицательный, не скроллим
    if (index < 0) return;

    // Обновляем ref ПОСЛЕ всех проверок
    lastProcessedViewDateRef.current = viewDateValue;

    // Ограничиваем индекс допустимым диапазоном
    const clampedIndex = Math.min(index, Math.max(0, datesLength - 1));

    // Выполняем скролл к вычисленной позиции
    const targetScrollX = distances.columnWidth * clampedIndex;
    setScrollXProgrammatically(targetScrollX);
  ...
  const handleEditTask = useCallback(
    (task: TaskOrEmpty) => {
      ...
      if (onEditTaskClick) {
        ...
        void (async () => {
          const nextTask = await onEditTask(task);
          if (!nextTask) {
            return;
          }
  ...
  const handleAddTask = useCallback(
    (task: Task) => {
      if (onAddTaskClick) {
        ...
        void (async () => {
          const nextTask = await onAddTask(task);
          if (!nextTask) {
            return;
          }
  ...
  return (
    ...
    >
      ...
      <div className={styles.ganttArea}>
        {/* Кастомный хедер рендерится вне scroll-контейнера для sticky поведения */}
        {customHeaderElement}
