  ...
  const recountOnMove = useCallback(
    (nextX: number) => {
      ...
      setChangeInProgress(prevValue => {
        ...
        const { changedTask: rawChangedTask } = handleTaskBySVGMouseEvent(
          prevValue.action,
          task,
        ...
        // TOOLTIP-SNAP: Применяем округление сразу, чтобы тултип показывал только "жёсткие" даты
        const newChangedTask = roundTaskDates(
          rawChangedTask,
          roundDate,
          prevValue.action,
          dateMoveStep
    ...
    [changeInProgress, rtl, svgWidth, roundDate, dateMoveStep]
  );
          ...
          case "move":
            if (scrollX > 0) {
              ...
              setChangeInProgress(prevValue => {
                ...
                const { changedTask: rawChangedTask } =
                ...
                // TOOLTIP-SNAP: Применяем округление для авто-скролла влево
                const newChangedTask = roundTaskDates(
                  rawChangedTask,
                  roundDate,
                  prevValue.action,
          ...
          case "move":
            if (svgWidth > scrollX + svgClientWidth) {
              ...
              setChangeInProgress(prevValue => {
                ...
                // TOOLTIP-SNAP: Применяем округление для авто-скролла вправо
                const newChangedTask = roundTaskDates(
                  rawChangedTask,
                  roundDate,
                  prevValue.action,
