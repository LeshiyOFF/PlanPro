...
try {
    ...
    Write-EnhancedLog "Attempting graceful process termination (Safe Mode)..." -Level "INFO"
    Stop-BlockingProcesses -ProcessNames @("ProjectLibre", "electron", "electron-builder", "java")
    ...
    for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
        ...
        # Check for ONLY relevant competing processes
        # Исключаем 'node', 'code', 'vscode', 'cursor' из критической проверки
        $allProcesses = Get-Process | Where-Object { 
            $_.ProcessName -match "ProjectLibre|electron|electron-builder|ant" 
        ...
        if ($allProcesses) {
            ...
            if ($remainingProcesses.Count -gt 0) {
                ...
                if ($attempt -eq $maxRetries) {
                    Write-EnhancedLog "WARNING: Some processes are persistent, but attempting to continue..." -Level "WARN"
                }
            } else {
                Write-EnhancedLog "All critical competing processes terminated" -Level "SUCCESS"
                break
            }
        } else {
            Write-EnhancedLog "No critical competing processes found" -Level "SUCCESS"
            break
        }
    }
